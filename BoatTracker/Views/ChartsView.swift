...
struct ChartRepresentable: UIViewRepresentable {
    ...
    class Coordinator {
        ...
        init(lang: TrackLang, isBoat: Bool) {
            ...
            blueDataSet = LineChartDataSet(entries: [], label: "\(isBoat ? lang.depth : lang.env.altitude) (m)")
        ...
        func updateDataSet(event: CoordsData) {
            ...
            let speedEntries = event.coords.map { c in
                ChartDataEntry(x: Double(c.time.millis), y: isBoat ? c.speed.knots : c.speed.kph, data: c.speed as AnyObject)
            }
            ...
            let altitudeEntries: [ChartDataEntry] = event.coords.compactMap { c in
                ...
                return ChartDataEntry(x: Double(c.time.millis), y: altitude.meters, data: c.altitude as AnyObject)
            }
