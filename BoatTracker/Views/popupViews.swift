...
struct LimitView: View {
    ...
    func items() -> [InfoItem] {
        [ InfoItem(lang.limits.limit, limit.types.map { $0.translate(lang: lang.limits.types) }.joined(separator: ", ")) ] +
        speedItem().toList +
        fairwayItem().toList
    }
...
struct BoatView: View {
    ...
    var body: some View {
        InfoView(title: from.boatName.name, items: items(), footer: info.coord.time.dateTime, leftColumnSize: .fixed(80))
    }
}
...
struct VesselView: View {
    ...
    var body: some View {
        InfoView(title: vessel.name, items: items(), footer: vessel.time.dateTime, leftColumnSize: .fixed(100))
    }
}
...
struct InfoView: View {
    ...
    init(title: String? = nil, items: [InfoItem] = [], footer: String? = nil, leftColumnSize: GridItem.Size = .flexible(), gridAlignment: HorizontalAlignment = .leading) {
        self.title = title
        self.items = items
        self.footer = footer
