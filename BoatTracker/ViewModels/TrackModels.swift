...
class ActiveTrack: ObservableObject {
  ...
  func clearIfOld() {
    guard let selected = selectedTrack, let name = selected.track else { return }
    let ageSeconds: TimeInterval = Date.now.timeIntervalSince(selected.selectedAt)
    let oneHour: TimeInterval = 3600
    if ageSeconds > oneHour {
      log.info("Track \(name) no longer fresh after \(ageSeconds) seconds. Clearing state.")
      select(nil)
    } else {
      log.info("Track \(name) still fresh after \(ageSeconds) seconds. Keeping it.")
    }
  }
}
