...
class RouteLayers {
  static func empty(style: Style) -> RouteLayers {
    RouteLayers(
      ...
      finish: LayerSource(
        iconId: "route-finish", iconImageName: Layers.routeEndIcon, iconSize: 0.04),
      style: style)
  }
  ...
  func update(
    initial: [CLLocationCoordinate2D], fairways: [CLLocationCoordinate2D],
...
class PathFinder: NSObject, UIGestureRecognizerDelegate {
  ...
  @MainActor func update(route: RouteResult) {
    ...
    do {
      try layers.update(
        initial: [route.from, fairwayPath.first ?? route.to],
      ...
      let camera = mapView.mapboxMap.camera(
        for: coords, padding: self.edgePadding, bearing: nil, pitch: nil)
