...
struct ShipTypesLang: Codable {
  ...
  let wingInGround, highSpeedCraft, pilotVessel, searchAndRescue, searchAndRescueAircraft,
    portTender, antiPollution, lawEnforce, localVessel, medicalTransport, specialCraft, passenger,
    cargo, tanker, other, unknown: String
}
...
struct FairwayTypesLang: Codable {
  let navigation, anchoring, meetup, harborPool, turn, channel, coastTraffic, core, special, lock,
    confirmedExtra, helcom, pilot: String
}
...
struct TrackLang: Codable {
  let track, boats, tracks, days, routes, speed, water, depth, top, duration, distance, topSpeed,
    avgSpeed, temperature, waterTemp, date, trackHistory, graph: String
  let env: EnvLang
}
...
struct AidTypeLang: Codable {
  let unknown, lighthouse, sectorLight, leadingMark, directionalLight, minorLight, otherMark,
    edgeMark, radarTarget, buoy, beacon, signatureLighthouse, cairn: String
}

struct NavMarkLang: Codable {
  let left, right, north, south, west, east, rock, safeWaters, special, notApplicable,
    unknown: String
}

struct ConstructionLang: Codable {
  let buoyBeacon, iceBuoy, beaconBuoy, superBeacon, exteriorLight, dayBoard, helicopterPlatform,
    radioMast, waterTower, smokePipe, radarTower, churchTower, superBuoy, edgeCairn, compassCheck,
    borderMark, borderLineMark, channelEdgeLight, tower: String
}
...
struct ProfileLang: Codable {
  let username, signedInAs, logout, chooseIdentityProvider, language, finnish, swedish, english,
    deleteAccount, deleteAccountConfirmation: String
}
...
struct SettingsLang: Codable {
  let appIcon, welcome, welcomeText, laterText, notifications, notificationsText, howItWorks,
